# Name:    MelissaProfilerObjectWindowsDotnet
# Purpose: Use the Melissa Updater to make the MelissaProfilerObjectWindowsDotnet code usable
...
$ProjectPath = "$CurrentPath\MelissaProfilerObjectWindowsDotnet"
...
Write-Host "`n========================== Melissa Profiler Object ==========================`n                         [ .NET | Windows | 64BIT ]`n"
...
dotnet publish -f="net8.0" -c Release -o $BuildPath MelissaProfilerObjectWindowsDotnet\MelissaProfilerObjectWindowsDotnet.csproj

# Run project
if ([string]::IsNullOrEmpty($file) ) {
  Push-Location MelissaProfilerObjectWindowsDotnet
  dotnet $BuildPath\MelissaProfilerObjectWindowsDotnet.dll --license $License  --dataPath $DataPath
  Pop-Location
}
else {
  Push-Location MelissaProfilerObjectWindowsDotnet
  dotnet $BuildPath\MelissaProfilerObjectWindowsDotnet.dll --license $License  --dataPath $DataPath --file $file
  Pop-Location
}
